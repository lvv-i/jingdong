...
| # | 任务 | 计划 | 产出物 | 状态 |
|---|------|------|--------|------|
| C1 | 工程初始化（Vite + Vue3 + Element Plus + 路由/状态/请求基建） | Day 1 | `admin-web/` 工程骨架 | ✅ 已完成（2026-08-15，build 通过） |
| C2 | 登录与角色化布局（登录页、路由守卫、动态菜单、Token 拦截） | Day 1 | `admin-web/src/` 基建 | ✅ 已完成（2026-08-15，browser-use 实测） |
| C3 | 商家后台 6 页（仪表盘/商品/库存/订单/售后/店铺） | Day 2-3 | M-001~M-015 全部页面 | ✅ 已完成（2026-08-15，browser-use 实测 6 页 + 接口核对表 15/15） |
| C4 | 管理员后台 7 页（审核/类目/巡检/商家/用户/争议/日志统计） | Day 3-4 | A-001~A-019 全部页面 | ✅ 已完成（2026-08-15，browser-use 实测 7 页 + 接口核对表 18/19） |
| C5 | 状态机与权限联动（按钮显隐、越权提示、三方协作链路自测） | Day 4 | 交互验收清单 | ✅ 已完成（2026-08-15，13 页映射表 + 越权拦截实测 6 用例全通过） |
| C6 | 联调与验收（与 A 后端联调、空状态处理、进度与实验记录归档） | Day 5 | 联调记录 + 演示脚本 | ✅ 已完成（2026-08-15，34 接口汇总核对 + 演示脚本 + AI 记录归档；真实联调待 M3） |

图例：⬜ 未开始 ｜ 🟦 进行中 ｜ ✅ 已完成　**C 第四阶段 6/6（全部完成）**
