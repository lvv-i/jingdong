...
|---|------|------|--------------------------|------|
| W1 | 工程初始化：Vite + Vue 3 + Element Plus + Router/Pinia/Axios + 登录注册 + 路由守卫 | Day 1 | `W-01-工程初始化.md`（登录/注册页 + 公共请求层 + 路由骨架） | ✅ 已完成 2026-08-15 |
| W2 | 商品浏览链路：首页 / 搜索列表 / 商品详情 / 评价列表（P-003~P-006） | Day 1-2 | `W-02-商品浏览链路.md` | ✅ 已完成 2026-08-15 |
| W3 | 购物与下单链路：购物车 / 结算 / 提交订单 / 模拟支付（U-008~U-016） | Day 2-3 | `W-03-购物与下单链路.md` | ✅ 已完成 2026-08-15 |
| W4 | 订单与售后链路：订单中心 / 订单详情 / 售后中心 / 评价（U-013~U-021/U-024） | Day 3-4 | `W-04-订单与售后链路.md` | ✅ 已完成 2026-08-15 |
| W5 | 个人中心链路 + 全链路自测：个人资料 / 地址 / 通知（U-001~U-007/U-022~U-025）+ 空状态与错误提示 | Day 4-5 | `W-05-个人中心与自测.md` | ✅ 已完成 2026-08-15 |
| W6 | 契约核对补充（不依赖 M3）：33 接口逐条核对表 + 注释对齐 + 偏差反馈清单 | 补充轮 | `W-06-接口契约核对表.md` | ✅ 已完成 2026-08-29 |

图例：⬜ 未开始 ｜ 🟦 进行中 ｜ ✅ 已完成　**B 第四阶段 5/5（W1~W5）+ W6 契约对齐补充（2026-08-29）**

> W6 要点：B 端消费方式与后端实现一致（U-003/U-008 裸数组、U-012 data.orders），0 功能级改动；修正 `api/auth.js`（P-008/U-002）、`api/refund.js`（U-018）、`api/order.js`（U-012/U-015/U-024）注释漂移；契约偏差清单待反馈 A 升版 T5 v1.1。前端工程 `frontend-user/` build 通过。
