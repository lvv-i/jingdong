...
- [x] token 结构（claims）、有效期、过期后的处理（1002 重新登录）明确
- [x] 拦截矩阵覆盖 /api/、/api/merchant/、/api/admin/ 三个前缀
- [x] 明确"前端隐藏按钮 + 后端强制校验"双保险原则
- [x] 敏感操作（退款处理、商家审核）约定写入 audit_logs

## 状态

- [ ] 未开始　[ ] 进行中　[x] 已完成
- 进展：2026-08-12 发布 v1.0；与 C 的权限矩阵对齐约定见交付物第 7 节
